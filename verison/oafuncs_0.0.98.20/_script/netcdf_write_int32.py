...
INT_FILL_VALUE = np.iinfo(np.int32).min
...
def _calculate_scale_and_offset(data, n=32, fill_value=INT_FILL_VALUE):
    """
...
def _data_to_scale_offset(data, scale, offset, fill_value=INT_FILL_VALUE):
    """
    ...
    if np.any(valid_mask):
        ...
        scaled = np.clip(scaled, fill_value + 1, fill_value * (-1))  # 不使用 -2147483648，保留做 _FillValue
    ...
    if isinstance(data, (xr.DataArray, xr.Dataset)):
        ...
        if isinstance(data, xr.DataArray):
            ...
            if np.issubdtype(arr.dtype, np.number) and scale_offset_switch:
                ...
                scale, offset = _calculate_scale_and_offset(arr_valid)
                # 写入前处理无效值（只在这里做！）
                arr_to_save = arr.copy()
                # 处理自定义缺失值
                if data_missing_val is not None:
                    arr_to_save[arr == data_missing_val] = INT_FILL_VALUE
                # 处理 NaN/inf
                arr_to_save[~np.isfinite(arr_to_save)] = INT_FILL_VALUE
        ...
        with nc.Dataset(file, mode, format="NETCDF4") as ncfile:
            ...
            if is_numeric and scale_offset_switch:
                ...
                if not np.any(valid_mask):
                    ...
                    # 确保没有 NaN
                    clean_data = np.nan_to_num(data, nan=missing_value if missing_value is not None else INT_FILL_VALUE)
                    var[:] = clean_data
                    return
                ...
                # 创建变量并设置属性
                var = ncfile.createVariable(varname, "i4", dims, zlib=compile_switch)
                var.scale_factor = scale
                var.add_offset = offset
                var._FillValue = INT_FILL_VALUE  # 明确设置填充值
                var[:] = new_data
            else:
                dtype = _numpy_to_nc_type(data.dtype)
                var = ncfile.createVariable(varname, dtype, dims, zlib=False)
                ...
                if np.issubdtype(data.dtype, np.floating) and np.any(~np.isfinite(data)):
                    fill_val = missing_value if missing_value is not None else INT_FILL_VALUE
...
def compress_netcdf(src_path, dst_path=None, tolerance=1e-10, preserve_mask_values=True):
    """
