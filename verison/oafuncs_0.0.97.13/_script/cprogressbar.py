...
class ColorProgressBar:
    ...
    def render(self) -> str:
        ...
        with self._lock:
            ...
            time_info = f"ETA: {remaining:.1f}s" if task.total and remaining and remaining > 0 else f"Elapsed: {elapsed:.1f}s"

            # 构建完整的进度条行
            line = f"{self.description} {current_color}[{bar}]{reset_code} {count_info} {percent} [{time_info} | {rate_info}]"
    ...
    def refresh(self) -> None:
        ...
        with self._lock:
            ...
            # 根据环境选择不同的输出方式
            if self._is_jupyter or not self._is_terminal:
                # Jupyter或非交互式环境，使用换行模式
...
if __name__ == "__main__":
    ...
    for _ in ColorProgressBar(range(100), description="Diverging:", cmap=cmap):
        time.sleep(0.1)
