...
def _download_within_python_requests(file_name, target_url, save_path):
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            print(f"[#3dfc40]File [bold #dfff73]{save_path} [#3dfc40]has been downloaded successfully, Time: [#39cbdd]{elapsed}")
...
def _download_within_python(file_name, target_url, save_path):
    print(f"[bold #f0f6d0]Requesting {file_name} ...")
...
def _download_file(target_url, store_path, file_name, cover=False):
    ...
    if not use_idm:
        _download_within_python(file_name, target_url, save_path)
